...
def parse_arguments():
    ...
    model_grp.add_argument("--device", default="cuda", help="Processing device for AI model inference. 'auto' automatically selects best available device, 'cuda' uses NVIDIA GPU (fastest), 'cpu' uses processor (universally compatible), 'intel-igpu' uses Intel integrated graphics, 'intel-dgpu' uses Intel discrete GPU, 'intel-npu' uses Intel Neural Processing Unit, 'amd-gpu' uses AMD GPU with OpenVINO (recommended for AMD on Windows). GPU acceleration significantly improves processing speed. Falls back to CPU if specified device unavailable.")
